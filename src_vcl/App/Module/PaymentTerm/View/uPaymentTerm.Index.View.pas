...
type
  TPaymentTermIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IPaymentTermMTB; AEntityState: TEntityState);
...
var
  PaymentTermIndexView: TPaymentTermIndexView;
...
uses
  uHlp,
  System.StrUtils,
  uPaymentTerm.CreateUpdate.View,
  uNotificationView,
  uSession.DTM,
  uYesOrNo.View,
...
procedure TPaymentTermIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IPaymentTermMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TPaymentTermIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Condicao de Pagamentos';
  pnlAppend.Width            := 0;
...
procedure TPaymentTermIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TPaymentTermIndexView.btnAppendClick(Sender: TObject);
var
  lStored: IPaymentTermMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TPaymentTermCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TPaymentTermIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TPaymentTermService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TPaymentTermIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: IPaymentTermMTB;
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TPaymentTermCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TPaymentTermIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TPaymentTermIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TPaymentTermIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentTermIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentTermIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TPaymentTermCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TPaymentTermIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TPaymentTermIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'payment_term.name';
...
procedure TPaymentTermIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TPaymentTermIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentTermIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
procedure TPaymentTermIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TPaymentTermIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'payment_term.'+lSelectedColumn;
...
procedure TPaymentTermIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TPaymentTermService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TPaymentTermIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TPaymentTermIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TPaymentTermIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('payment_term.id',                 'ID', True)
    .AddColumn ('payment_term.name',               'Nome', True)
    .AddColumn ('payment_term.created_at',         'Criado em (Data)', False)
    .AddColumn ('payment_term.updated_at',         'Atualizado em (Data)', False)
...
procedure TPaymentTermIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TPaymentTermIndexView.HandleLocate: Int64;
var
  lView: TPaymentTermIndexView;
begin
  Try
    lView := TPaymentTermIndexView.Create(nil);
...
procedure TPaymentTermIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TPaymentTermIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TPaymentTermIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TPaymentTermIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TPaymentTermIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TPaymentTermIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TPaymentTermIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
