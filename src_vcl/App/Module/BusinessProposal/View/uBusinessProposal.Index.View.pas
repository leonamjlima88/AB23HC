unit uBusinessProposal.Index.View;
...
type
  TBusinessProposalIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
    ...
    pnlGridLegend: TPanel;
    Panel5: TPanel;
    Panel6: TPanel;
    Panel7: TPanel;
    mniBusinessProposalPrint: TMenuItem;
    N2: TMenuItem;
    mniBusinessProposalPrintA4: TMenuItem;
    ...
    procedure mniBusinessProposalPrintA4Click(Sender: TObject);
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IBusinessProposalMTB; AEntityState: TEntityState);
...
var
  BusinessProposalIndexView: TBusinessProposalIndexView;
...
uses
  uHlp,
  System.StrUtils,
  uBusinessProposal.CreateUpdate.View,
  uNotificationView,
  uSession.DTM,
  uYesOrNo.View,
...
procedure TBusinessProposalIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IBusinessProposalMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TBusinessProposalIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Propostas Comerciais';
  pnlAppend.Width            := 0;
...
procedure TBusinessProposalIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TBusinessProposalIndexView.btnAppendClick(Sender: TObject);
var
  lStored: IBusinessProposalMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TBusinessProposalCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TBusinessProposalIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TBusinessProposalService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TBusinessProposalIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: IBusinessProposalMTB;
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TBusinessProposalCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TBusinessProposalIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TBusinessProposalIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TBusinessProposalIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TBusinessProposalIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TBusinessProposalIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TBusinessProposalCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TBusinessProposalIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TBusinessProposalIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'business_proposal.id';
...
procedure TBusinessProposalIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
  lMenuItem: TMenuItem;
begin
  ...
  begin
    for lMenuItem in ppmOptions.Items do
    begin
      if not MatchStr(lMenuItem.Name, [
        'mniBusinessProposalPrint',
        'mniBusinessProposalPrintA4'
      ]) then
...
procedure TBusinessProposalIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TBusinessProposalIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
  ...
  begin
    TDBGrid(Sender).Canvas.FillRect(Rect);
    SessionDTM.imgListGrid.Draw(TDBGrid(Sender).Canvas, Rect.Left +1,Rect.Top + 1, 3);
  end;
  ...
  begin
    ...
    case dtsIndex.DataSet.FieldByName('status').AsInteger of
      0: SessionDTM.imgListGrid.Draw(TDBGrid(Sender).Canvas, Rect.Left +1,Rect.Top + 1, 7);
      1: SessionDTM.imgListGrid.Draw(TDBGrid(Sender).Canvas, Rect.Left +1,Rect.Top + 1, 8);
      2: SessionDTM.imgListGrid.Draw(TDBGrid(Sender).Canvas, Rect.Left +1,Rect.Top + 1, 9);
    end;
  end;
end;

procedure TBusinessProposalIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TBusinessProposalIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'business_proposal.'+lSelectedColumn;
...
procedure TBusinessProposalIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TBusinessProposalService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TBusinessProposalIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TBusinessProposalIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TBusinessProposalIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('business_proposal.id',                  'ID', True)
    .AddColumn ('business_proposal.created_at',          'Criado em (Data)', False)
    .AddColumn ('business_proposal.updated_at',          'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name',              'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name',              'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID');
...
procedure TBusinessProposalIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TBusinessProposalIndexView.HandleLocate: Int64;
var
  lView: TBusinessProposalIndexView;
begin
  Try
    lView := TBusinessProposalIndexView.Create(nil);
...
procedure TBusinessProposalIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TBusinessProposalIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TBusinessProposalIndexView.imgOptionsClick(Sender: TObject);
var
  lMenuItem: TMenuItem;
begin
  inherited;
...
procedure TBusinessProposalIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TBusinessProposalIndexView.mniBusinessProposalPrintA4Click(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
  lKeepGoing := Assigned(dtsIndex.DataSet) and dtsIndex.DataSet.Active and (dtsIndex.DataSet.RecordCount > 0);
  if not lKeepGoing then
    Exit;
  ...
  try
    ...
    TBusinessProposalService.Make.ReportById(dtsIndex.DataSet.FieldByName('id').AsLargeInt);
  finally
    pnlBackground.Enabled := True;
...
procedure TBusinessProposalIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TBusinessProposalIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TBusinessProposalIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
