...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ISizeMTB; AEntityState: TEntityState);
...
procedure TSizeIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ISizeMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TSizeCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TSizeCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TSizeCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TSizeIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TSizeService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('size.id',                 'ID', True)
    .AddColumn ('size.name',               'Nome', True)
    .AddColumn ('size.created_at',         'Criado em (Data)', False)
    .AddColumn ('size.updated_at',         'Atualizado em (Data)', False)
...
var
  lView: TSizeIndexView;
begin
  Try
    lView := TSizeIndexView.Create(nil);
