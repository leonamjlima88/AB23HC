...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IBankMTB; AEntityState: TEntityState);
...
procedure TBankIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IBankMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TBankCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TBankCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TBankCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TBankIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TBankService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('bank.id',                 'ID', True)
    .AddColumn ('bank.name',               'Nome', True)
    .AddColumn ('bank.created_at',         'Criado em (Data)', False)
    .AddColumn ('bank.updated_at',         'Atualizado em (Data)', False)
...
var
  lView: TBankIndexView;
begin
  Try
    lView := TBankIndexView.Create(nil);
