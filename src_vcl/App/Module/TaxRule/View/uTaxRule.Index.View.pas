...
type
  TTaxRuleIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ITaxRuleMTB; AEntityState: TEntityState);
...
var
  TaxRuleIndexView: TTaxRuleIndexView;
...
procedure TTaxRuleIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ITaxRuleMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TTaxRuleIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Regras Fiscais';
  pnlAppend.Width            := 0;
...
procedure TTaxRuleIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TTaxRuleIndexView.btnAppendClick(Sender: TObject);
var
  lStored: ITaxRuleMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TTaxRuleCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TTaxRuleIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TTaxRuleService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TTaxRuleIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: ITaxRuleMTB;
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TTaxRuleCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TTaxRuleIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TTaxRuleIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TTaxRuleIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TTaxRuleIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TTaxRuleIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TTaxRuleCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TTaxRuleIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TTaxRuleIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TTaxRuleIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TTaxRuleIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TTaxRuleIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
procedure TTaxRuleIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TTaxRuleIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'tax_rule.'+lSelectedColumn;
...
procedure TTaxRuleIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TTaxRuleService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TTaxRuleIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TTaxRuleIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TTaxRuleIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('tax_rule.id',              'ID', True)
    .AddColumn ('tax_rule.name',            'Nome', True)
    .AddColumn ('tax_rule.created_at',      'Criado em (Data)', False)
    .AddColumn ('tax_rule.updated_at',      'Atualizado em (Data)', False)
...
procedure TTaxRuleIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TTaxRuleIndexView.HandleLocate: Int64;
var
  lView: TTaxRuleIndexView;
begin
  Try
    lView := TTaxRuleIndexView.Create(nil);
...
procedure TTaxRuleIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TTaxRuleIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TTaxRuleIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TTaxRuleIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TTaxRuleIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TTaxRuleIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TTaxRuleIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
