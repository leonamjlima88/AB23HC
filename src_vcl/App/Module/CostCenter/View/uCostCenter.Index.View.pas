...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ICostCenterMTB; AEntityState: TEntityState);
...
procedure TCostCenterIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ICostCenterMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Centro de Custos';
  pnlAppend.Width            := 0;
...
begin
  ...
  Try
    ...
    lStored := TCostCenterCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TCostCenterCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TCostCenterCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TCostCenterIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TCostCenterService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('cost_center.id',                 'ID', True)
    .AddColumn ('cost_center.name',               'Nome', True)
    .AddColumn ('cost_center.created_at',         'Criado em (Data)', False)
    .AddColumn ('cost_center.updated_at',         'Atualizado em (Data)', False)
...
var
  lView: TCostCenterIndexView;
begin
  Try
    lView := TCostCenterIndexView.Create(nil);
