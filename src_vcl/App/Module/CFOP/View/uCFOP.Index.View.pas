...
type
  TCFOPIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ICFOPMTB; AEntityState: TEntityState);
...
procedure TCFOPIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ICFOPMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TCFOPIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... CFOPs';
  pnlAppend.Width            := 0;
...
procedure TCFOPIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TCFOPIndexView.btnAppendClick(Sender: TObject);
var
  lStored: ICFOPMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TCFOPCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TCFOPIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TCFOPService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TCFOPIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: ICFOPMTB;
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TCFOPCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TCFOPIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TCFOPIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TCFOPIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TCFOPIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TCFOPIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TCFOPCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TCFOPIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TCFOPIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TCFOPIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TCFOPIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TCFOPIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
procedure TCFOPIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TCFOPIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'cfop.'+lSelectedColumn;
...
procedure TCFOPIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TCFOPService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TCFOPIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TCFOPIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TCFOPIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('cfop.id',                 'ID', True)
    .AddColumn ('cfop.name',               'Nome', True)
    .AddColumn ('cfop.code',               'CFOP', True)
    .AddColumn ('cfop.created_at',         'Criado em (Data)', False)
    .AddColumn ('cfop.updated_at',         'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name', 'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name', 'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID, Nome ou CFOP');
...
procedure TCFOPIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TCFOPIndexView.HandleLocate: Int64;
var
  lView: TCFOPIndexView;
begin
  Try
    lView := TCFOPIndexView.Create(nil);
...
procedure TCFOPIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TCFOPIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TCFOPIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TCFOPIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TCFOPIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TCFOPIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TCFOPIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
