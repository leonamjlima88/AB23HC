...
type
  TDocumentIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IDocumentMTB; AEntityState: TEntityState);
...
var
  DocumentIndexView: TDocumentIndexView;
...
procedure TDocumentIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IDocumentMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TDocumentIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Documentos';
  pnlAppend.Width            := 0;
...
procedure TDocumentIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TDocumentIndexView.btnAppendClick(Sender: TObject);
var
  lStored: IDocumentMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TDocumentCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TDocumentIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TDocumentService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TDocumentIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: IDocumentMTB;
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TDocumentCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TDocumentIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TDocumentIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TDocumentIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TDocumentIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TDocumentIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TDocumentCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TDocumentIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TDocumentIndexView.CleanFilter;
begin
  // Modo de Pesquisa
...
procedure TDocumentIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TDocumentIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TDocumentIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
procedure TDocumentIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TDocumentIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'document.'+lSelectedColumn;
...
procedure TDocumentIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TDocumentService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TDocumentIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TDocumentIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TDocumentIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('document.id',                 'ID', True)
    .AddColumn ('document.name',               'Nome', True)
    .AddColumn ('document.created_at',         'Criado em (Data)', False)
    .AddColumn ('document.updated_at',         'Atualizado em (Data)', False)
...
procedure TDocumentIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TDocumentIndexView.HandleLocate: Int64;
var
  lView: TDocumentIndexView;
begin
  Try
    lView := TDocumentIndexView.Create(nil);
...
procedure TDocumentIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TDocumentIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TDocumentIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TDocumentIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TDocumentIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TDocumentIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TDocumentIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
