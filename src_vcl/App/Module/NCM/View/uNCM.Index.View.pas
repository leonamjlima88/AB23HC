...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: INCMMTB; AEntityState: TEntityState);
...
procedure TNCMIndexView.RefreshIndexWithoutRequestAPI(AMemTable: INCMMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... NCMs';
  pnlAppend.Width            := 0;
...
begin
  ...
  Try
    ...
    lStored := TNCMCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TNCMCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TNCMCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TNCMIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TNCMService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('ncm.id',                   'ID', True)
    .AddColumn ('ncm.name',                 'Nome', True)
    .AddColumn ('ncm.ncm',                  'NCM', True)
    .AddColumn ('ncm.created_at',           'Criado em (Data)', False)
    .AddColumn ('ncm.updated_at',           'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name', 'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name', 'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID, Nome ou NCM');
...
begin
  Try
    lView := TNCMIndexView.Create(nil);
