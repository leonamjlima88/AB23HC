...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ICityMTB; AEntityState: TEntityState);
...
procedure TCityIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ICityMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TCityCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TCityCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TCityCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TCityIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TCityService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('city.id',                  'ID', True)
    .AddColumn ('city.name',                'Nome', True)
    .AddColumn ('city.state',               'UF', True)
    .AddColumn ('city.ibge_code',           'Cód IBGE da Cidade', False)
    .AddColumn ('city.created_at',          'Criado em (Data)', False)
    .AddColumn ('city.updated_at',          'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name', 'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name', 'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID, Nome ou UF');
...
var
  lView: TCityIndexView;
begin
  Try
    lView := TCityIndexView.Create(nil);
