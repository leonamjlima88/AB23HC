unit uOperationType.Index.View;
...
type
  TOperationTypeIndexView = class(TBaseIndexView)
    tmrDoSearch: TTimer;
    pnlLocate: TPanel;
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IOperationTypeMTB; AEntityState: TEntityState);
...
var
  OperationTypeIndexView: TOperationTypeIndexView;
...
uses
  uHlp,
  System.StrUtils,
  uOperationType.CreateUpdate.View,
  uNotificationView,
  uSession.DTM,
  uYesOrNo.View,
...
procedure TOperationTypeIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IOperationTypeMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
procedure TOperationTypeIndexView.SetLayoutLocate(ABackgroundTransparent: Boolean);
const
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Tipos de operação';
  pnlAppend.Width            := 0;
...
procedure TOperationTypeIndexView.SetLocateResult(const Value: Integer);
begin
  FLocateResult := Value;
end;

procedure TOperationTypeIndexView.btnAppendClick(Sender: TObject);
var
  lStored: IOperationTypeMTB;
begin
  inherited;
  ...
  Try
    ...
    lStored := TOperationTypeCreateUpdateView.Handle(TEntityState.esStore);
...
procedure TOperationTypeIndexView.btnDeleteClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  Try
    ...
    if not TOperationTypeService.Make.Delete(dtsIndex.DataSet.Fields[0].AsLargeInt) then
    begin
...
procedure TOperationTypeIndexView.btnEditClick(Sender: TObject);
var
  lKeepGoing: Boolean;
  lUpdated: IOperationTypeMTB;
begin
  // Evitar erros
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TOperationTypeCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
procedure TOperationTypeIndexView.btnLocateCloseClick(Sender: TObject);
begin
  inherited;
  FLocateResult := -1;
...
procedure TOperationTypeIndexView.btnLocateConfirmClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TOperationTypeIndexView.btnNavigationClick(Sender: TObject);
begin
  inherited;
...
procedure TOperationTypeIndexView.btnSplitViewApplyClick(Sender: TObject);
begin
  inherited;
...
procedure TOperationTypeIndexView.btnViewClick(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
  ...
  // Visualizar Registro
  TOperationTypeCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;

procedure TOperationTypeIndexView.cbxFilterIndexSelect(Sender: TObject);
begin
  inherited;
  lblSearchTitle.Caption := 'F5 - Pesquise por: "' + cbxFilterIndex.Text + '"';
end;

procedure TOperationTypeIndexView.CleanFilter;
begin
  // Modo de Pesquisa
  ...
  // Ordenar por
  FFilterOrderBy := 'operation_type.name';
...
procedure TOperationTypeIndexView.DBGrid1CellClick(Column: TColumn);
var
  lKeepGoing: Boolean;
begin
  inherited;
...
procedure TOperationTypeIndexView.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TOperationTypeIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
procedure TOperationTypeIndexView.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TOperationTypeIndexView.DBGrid1TitleClick(Column: TColumn);
const
  L_ACTIONS: TArray<String> = ['action_edit','action_delete','action_view','action_option'];
  L_DESC = ':D';
var
...
begin
  Try
    ...
    // OrderBy e Indexador
    FFilterOrderBy := 'operation_type.'+lSelectedColumn;
...
procedure TOperationTypeIndexView.DoSearch(ACurrentPage: Integer; ATryLocateId: Int64);
var
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TOperationTypeService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
procedure TOperationTypeIndexView.edtSearchValueChange(Sender: TObject);
begin
  inherited;
...
procedure TOperationTypeIndexView.edtSearchValueKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TOperationTypeIndexView.FormCreate(Sender: TObject);
begin
  inherited;

  // Colunas de Pesquisa da Tabela
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('operation_type.id',                 'ID', True)
    .AddColumn ('operation_type.name',               'Nome', True)
    .AddColumn ('operation_type.created_at',         'Criado em (Data)', False)
    .AddColumn ('operation_type.updated_at',         'Atualizado em (Data)', False)
...
procedure TOperationTypeIndexView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
class function TOperationTypeIndexView.HandleLocate: Int64;
var
  lView: TOperationTypeIndexView;
begin
  Try
    lView := TOperationTypeIndexView.Create(nil);
...
procedure TOperationTypeIndexView.imgDoSearchClick(Sender: TObject);
begin
  inherited;
  DoSearch;
end;

procedure TOperationTypeIndexView.imgFilterCleanClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
end;

procedure TOperationTypeIndexView.imgOptionsClick(Sender: TObject);
begin
  inherited;
  ppmOptions.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
end;

procedure TOperationTypeIndexView.imgSearchClearClick(Sender: TObject);
begin
  inherited;
  CleanFilter;
  DoSearch;
...
procedure TOperationTypeIndexView.mniDeleteGridConfigClick(Sender: TObject);
begin
  inherited;
  // Excluir Grid
...
procedure TOperationTypeIndexView.mniSaveGridConfigClick(Sender: TObject);
begin
  inherited;
...
procedure TOperationTypeIndexView.tmrDoSearchTimer(Sender: TObject);
begin
  inherited;
  tmrDoSearch.Enabled := False;
  DoSearch;
