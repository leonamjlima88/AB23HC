...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IChartOfAccountMTB; AEntityState: TEntityState);
...
uses
  uHlp,
  System.StrUtils,
  uChartOfAccount.CreateUpdate.View,
  uNotificationView,
  uSession.DTM,
  uYesOrNo.View,
...
procedure TChartOfAccountIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IChartOfAccountMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Plano de Contas';
  pnlAppend.Width            := 0;
...
begin
  ...
  Try
    ...
    lStored := TChartOfAccountCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TChartOfAccountCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Bloquear se não for analítico
  if not THlp.IntBool(dtsIndex.DataSet.FieldByName('is_analytical').AsInteger) then
  begin
    TAlertView.Handle('Apenas registros analíticos podem ser selecionados!');
    Abort;
  end;
...
begin
  ...
  // Visualizar Registro
  TChartOfAccountCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
begin
  ...
  // Ordenar por
  FFilterOrderBy := 'chart_of_account.hierarchy_code';
...
procedure TChartOfAccountIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
  ...
  if (Column.FieldName.ToLower = 'hierarchy_code') or
     (Column.FieldName.ToLower = 'name') then
  begin
    case dtsIndex.DataSet.FieldByName('is_analytical').AsInteger of
      0: Begin
        TDBGrid(Sender).Canvas.Font.Color := $00AFA47A;
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TChartOfAccountService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('chart_of_account.id',                 'ID', True)
    .AddColumn ('chart_of_account.name',               'Nome', True)
    .AddColumn ('chart_of_account.hierarchy_code',     'Hierarquia', True)
    .AddColumn ('chart_of_account.created_at',         'Criado em (Data)', False)
    .AddColumn ('chart_of_account.updated_at',         'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name', 'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name', 'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID, Nome ou Hierarquia');
...
var
  lView: TChartOfAccountIndexView;
begin
  Try
    lView := TChartOfAccountIndexView.Create(nil);
