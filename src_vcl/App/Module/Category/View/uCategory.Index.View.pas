...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ICategoryMTB; AEntityState: TEntityState);
...
procedure TCategoryIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ICategoryMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TCategoryCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TCategoryCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TCategoryCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TCategoryIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TCategoryService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('category.id',                 'ID', True)
    .AddColumn ('category.name',               'Nome', True)
    .AddColumn ('category.created_at',         'Criado em (Data)', False)
    .AddColumn ('category.updated_at',         'Atualizado em (Data)', False)
...
var
  lView: TCategoryIndexView;
begin
  Try
    lView := TCategoryIndexView.Create(nil);
