...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IBankAccountMTB; AEntityState: TEntityState);
...
uses
  uHlp,
  System.StrUtils,
  uBankAccount.CreateUpdate.View,
  uNotificationView,
  uSession.DTM,
  uYesOrNo.View,
...
procedure TBankAccountIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IBankAccountMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TBankAccountCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TBankAccountCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TBankAccountCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TBankAccountIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TBankAccountService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('bank_account.id',          'ID', True)
    .AddColumn ('bank_account.name',        'Nome', True)
    .AddColumn ('bank_account.created_at',  'Criado em (Data)', False)
    .AddColumn ('bank_account.updated_at',  'Atualizado em (Data)', False)
...
var
  lView: TBankAccountIndexView;
begin
  Try
    lView := TBankAccountIndexView.Create(nil);
