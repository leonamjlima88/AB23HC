...
type
  ...
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: IUnitMTB; AEntityState: TEntityState);
...
procedure TUnitIndexView.RefreshIndexWithoutRequestAPI(AMemTable: IUnitMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  lblTitle.Caption           := 'Pesquisando... Unidades de Medida';
  pnlAppend.Width            := 0;
...
begin
  ...
  Try
    ...
    lStored := TUnitCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TUnitCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TUnitCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
procedure TUnitIndexView.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
Var
  lI: Integer;
  lKeepGoing: Boolean;
begin
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TUnitService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('unit.id',                  'ID', True)
    .AddColumn ('unit.name',                'Nome', True)
    .AddColumn ('unit.description',         'Descrição', True)
    .AddColumn ('unit.created_at',          'Criado em (Data)', False)
    .AddColumn ('unit.updated_at',          'Atualizado em (Data)', False)
    .AddColumn ('created_by_acl_user.name', 'Criado por (Nome)', False)
    .AddColumn ('updated_by_acl_user.name', 'Atualizado por (Nome)', False)
    .LabelForCustomSearch ('ID, Nome ou Descrição');
...
var
  lView: TUnitIndexView;
begin
  Try
    lView := TUnitIndexView.Create(nil);
