...
type
  TSaleIndexView = class(TBaseIndexView)
    ...
    Panel6: TPanel;
    Panel7: TPanel;
    mniSalePrint: TMenuItem;
    N2: TMenuItem;
    mniSalePrintA4: TMenuItem;
    ...
    procedure mniSalePrintA4Click(Sender: TObject);
  private
    ...
    procedure RefreshIndexWithoutRequestAPI(AMemTable: ISaleMTB; AEntityState: TEntityState);
...
procedure TSaleIndexView.RefreshIndexWithoutRequestAPI(AMemTable: ISaleMTB; AEntityState: TEntityState);
var
  lKeepGoing: Boolean;
begin
  // Evitar erros
...
begin
  ...
  Try
    ...
    lStored := TSaleCreateUpdateView.Handle(TEntityState.esStore);
...
begin
  ...
  Try
    ...
    // Editar Registro
    lUpdated := TSaleCreateUpdateView.Handle(TEntityState.esUpdate, dtsIndex.DataSet.Fields[0].AsLargeInt);
...
begin
  ...
  // Visualizar Registro
  TSaleCreateUpdateView.Handle(TEntityState.esView, dtsIndex.DataSet.Fields[0].AsLargeInt);
end;
...
begin
  ...
  TRunTask.Execute(
    ...
    begin
      ...
      FIndexResult := TSaleService.Make.Index(lPageFilter);
    end)
  .OnException_Sync(
...
begin
  ...
  FSearchColumns := TSearchColumns.Make
    .AddColumn ('sale.id',                  'ID', True)
    .AddColumn ('sale.created_at',          'Criado em (Data)', False)
    .AddColumn ('sale.updated_at',          'Atualizado em (Data)', False)
...
var
  lView: TSaleIndexView;
begin
  Try
    lView := TSaleIndexView.Create(nil);
...
procedure TSaleIndexView.mniSalePrintA4Click(Sender: TObject);
var
  lKeepGoing: Boolean;
begin
  inherited;
  ...
  try
    ...
    TSaleService.Make.ReportById(dtsIndex.DataSet.FieldByName('id').AsLargeInt);
  finally
    pnlBackground.Enabled := True;
